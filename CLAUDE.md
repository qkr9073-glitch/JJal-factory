...
- **운영 vs 개발 분리**: 실제 크롤·인스타 게시·서버는 **사장님 PC(이 유일본)에서만** 돈다.
  다른 개발자는 자기 컴에서 **코드만** 만들어 **`git push`(main)** → **완전자동 배포**.
- **자동배포 = `autopull.pyw`**(무창, 포트 8779 잠금, 감시견이 관리): 30초마다 `git fetch origin main` → 원격이 앞서고 로컬이 clean이면 `merge --ff-only` → **`.py` 바뀌면 8777 kill(감시견이 새 코드로 부활)**, 문서만 바뀌면 재시작 생략. **가드**: 로컬 dirty(사장님 편집 중)면 pull 건너뜀, ff 불가(분기)면 자동병합 안 하고 로그만(logs/autopull.log). push→라이브 ~1분. 급하면 **`배포.cmd`**(즉시 pull+재시작). ⚠️**이 PC에서 코드 고치면 반드시 commit+push** 해야 autopull과 안 엉킴(안 하면 dirty로 자동배포 멈춤).
