...
{{
  ...
  "title_top": "표지 인용/배지용 짧은 후킹 한 줄 (따옴표 없이, 18자 이내). ⚠️'저장필수'·'팔로우' 같은 상용구 금지 — 렌더러가 저장 배지를 따로 붙인다. ⚠️'일본에서(도) 화제' 같은 실증 불가한 반응 주장 금지(형식 지침에 일본 화제 근거가 있을 때만 허용). 사건 속 대사나 궁금증 문구로",
  ...
  "image_query": "표지 AI 이미지 장면 묘사 — 영문, 글자 없는 장면(폰·간판 화면의 글자도 금지), 원본 사건을 시각화 (1~2문장). 먼저 표지가 유발할 감정을 정하고(경악/억울/폭소/부러움 등) 그 감정이 '극대화된 순간'을 묘사하라 — 원본 표지보다 감정이 약해지면 실패다. 장면 톤은 소재 결과 일치(유머·꿀팁=밝은 일상, 어두운 골목=사건·음지 전용). ⚠️영화·게임·연예 등 IP 소재면 캐릭터·코스튬·로고를 그리게 하지 마라(비슷하게 그려져도 저작권 위험) — 반드시 '장소·소품·군중' 장면으로만 (예: 영화 소재 → crowded movie theater lobby at night, glowing screens, popcorn / 진열대·티켓·네온 등). 인물은 익명의 일반인만",
...
def remake_build(cfg, base, handle, media_id, audience="", log=print):
    ...
    if audience:
        atxt = (f"🎯 수요층: 이 게시물의 최종 시청자는 {audience}다. 감정의 주어·당사자를 "
                ...
                f"없으면 지어내지 마라 — '그 나라에서도 화제' 같은 실증 불가한 반응 "
                f"주장도 금지, 대신 수요층의 궁금증·질문('일본 독자라면 몇 위?')으로 "
                f"당사자화하라. "
                ...
                f"치환한 소재에 '일본에서(도) 화제' 같은 실증 불가한 반응 주장을 붙이지 "
                f"마라 — 출처 주장 없이 소재 자체(랭킹·질문·궁금증)로 후킹한다.")
...
일본 항목이 원재료에 없으면 지어내지 마라 — "일본에서도 화제" 같은 실증 불가한 반응
주장도 금지(참고 정보에 일본 화제 근거가 있을 때만 허용). 대신 일본 시청자의 궁금증·질문
("일본 독자라면 몇 위?")으로 당사자화하라.
...
- 표지 카피(서브라인·배지 포함)도 사실 규칙을 따른다: 검증 불가한 절대 단정("일본에선
  절대 없다"류)과 실증 없는 반응 주장("일본에서도 화제" — 참고 정보에 근거 있을 때만)은
  금지. **표지의 주장이 본문 내용과 모순되면 실패다** (예: 표지 "일본엔 없다" ↔ 본문
  "일본과 같다고 생각했죠?").
...
{{
  ...
  "title_top": "표지 배지용 짧은 후킹 (18자 이내). ⚠️'저장필수'·'팔로우' 같은 상용구 금지 — 렌더러가 저장 배지를 따로 붙인다. ⚠️'일본에서(도) 화제' 같은 실증 불가한 반응 주장 금지(참고 정보에 일본 화제 근거가 있을 때만 허용)",
  "title_main": "표지 헤드라인 (22자 이내)",
  "subtitle": "서브라인 (부러움/충격 포인트, 25자 이내, 없으면 빈 문자열)",
  "image_query": "표지 AI 이미지 장면 묘사 — 영문, 글자 없는 장면 (1~2문장). 먼저 표지가 유발할 감정을 정하고(경악/억울/폭소/부러움 등) 그 감정이 '극대화된 순간'을 묘사하라 — 감정이 약한 밋밋한 장면이면 실패다. ⚠️장면의 톤은 소재의 결과 일치시켜라 — 가벼운 유머·꿀팁·참여형 소재는 밝은 일상 배경(카페·거실·거리 낮), 어두운 뒷골목·위협적 분위기는 사건·음지 소재 전용. ⚠️브랜드명·저작권 캐릭터·유명인 이름 금지 — 분위기·소품으로 우회 묘사. 폰·간판 등 화면에 글자를 그리게 하지 마라",
