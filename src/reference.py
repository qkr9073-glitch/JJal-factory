...
def remake_build(cfg, base, handle, media_id, audience="", log=print):
    ...
    if audience:
        atxt = (f"🎯 수요층: 이 게시물의 최종 시청자는 {audience}다. 감정의 주어·당사자를 "
                ...
                f"🏷 표지 카피에 콘텐츠 무대·화제 출처가 되는 나라 키워드를 자연스럽게 "
                f"박아라(일본 타겟이면 '한국'/「韓国」) — 벤치마크 표지 실측 62%가 국가 "
                f"키워드를 노출한다. 그 시그널 자체가 클릭 이유다.")
...
🏷 국가 키워드: 표지 카피(헤드라인 또는 서브라인)에 '한국'(일본어판 「韓国」)을 자연스럽게
박아라 — 벤치마크 채널 실측: 표지의 62%가 국가 키워드를 노출한다. 피드에서 "이건 한국
이야기"라는 시그널 자체가 이 채널의 클릭 이유다. 문장이 어색해지면 억지로 넣지는 마라.
